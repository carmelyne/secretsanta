...
function displayContent(json) {
    ...
    wish_list_html += [
      ...
      '">',
      'View Wishlist</a>&nbsp;',
      getGiftStatusIcons(json.feed.entry[i]),
      '<p>',
...
function getGiftStatusIcons(entry) {
    var iconString = '<span class="pull-right">';
    
    if ( entry.gsx$gaveagift.$t.length > 0 ) { // they were marked as having gave a gift in spreadsheet
        iconString += '<img src="./assets/img/ss-ico.png" title="gave a gift!"/> ';
    }
    
    if ( entry.gsx$giftreceivedorgiftincoming.$t.length > 0 ) { // they were marked as having a received a gift
